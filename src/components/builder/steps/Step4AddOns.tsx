    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-black-soft border border-gold/30 rounded-2xl p-6 max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          ...
          {SIZE_GUIDE.map((item) => (
            <>
              <div key={`size-${item.size}`} className="text-white-off">{item.size}</div>
              <div key={`circ-${item.size}`} className="text-white-off/60 col-span-2">{item.circumference} mm</div>
            </>
          ))}
        </div>
...
export function Step4AddOns() {
  const { engraving, setEngraving, ringSize, setRingSize, notes, setNotes, completeStep, setStep } = useBuilderStore()
  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-white-off/60 text-sm">Add the finishing touches to make it uniquely yours</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          >
            <option value="" className="bg-black-soft text-white-off/50">Select your ring size</option>
            {RING_SIZES.map((size) => (
        ...
        <div className="flex items-start gap-2 p-3 bg-black/40 border border-white/10 rounded-lg">
          ...
          <p className="text-white-off/50 text-xs leading-relaxed">
            Not sure of your size? No worries! You can finalize this during your consultation. 
            We also offer complimentary resizing within 30 days.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          <span className="text-white-off font-semibold tracking-wide text-sm">SPECIAL REQUESTS</span>
          <span className="text-white-off/40 text-xs">(Optional)</span>
        </div>
        ...
        >
          {saved ? (
            <>
              ...
              Design Saved!
              {designId && <span className="text-xs opacity-60 normal-case ml-1">({designId})</span>}
            </>
          ) : (
            <>
